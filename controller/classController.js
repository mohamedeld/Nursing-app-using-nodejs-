...
exports.getClass =async (request,response,next)=>{
    try{
        let classOne =  await Classes.findById(request.params.id);
        response.status(200).json({
            data:{
                classOne
            }
...
exports.updateClass =async (request,response,next)=>{
    try{
        let classUpdated = await Classes.findByIdAndUpdate(request.params.id,request.body,{
            new:true
        })
        response.status(200).json({
            data:{
...
exports.getClassesChildren = async (request,response,next)=>{
    try{
        let classChild = await classes.find({_id:request.params.id}).populate({
            path:"childrens"
...
exports.getTeacherSupervisor = async (request,response,next)=>{
    try{
        let teacherSupervisor = await classes.find({_id:request.params.id}).populate({
            path:"teachers"
